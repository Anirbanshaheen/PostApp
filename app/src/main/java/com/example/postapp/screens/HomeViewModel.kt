package com.example.postapp.screens
...
class HomeViewModel @Inject constructor(private val postRepository: PostRepository) : ViewModel() {
    ...
    fun getPostDetails(id: Int): LiveData<PostDetailsModel> {

        viewState.value = ViewState.ProgressState(true)
        val responseBody = MutableLiveData<PostDetailsModel>()
    ...
    fun getPostComment(id: Int): LiveData<PostCommentModel> {

        viewState.value = ViewState.ProgressState(true)
        val responseBody = MutableLiveData<PostCommentModel>()

        viewModelScope.launch(Dispatchers.IO) {
            val response = postRepository.getPostComment(id)
            withContext(Dispatchers.Main) {
                viewState.value = ViewState.ProgressState(false)
                when (response) {
                    is NetworkResponse.Success -> {
